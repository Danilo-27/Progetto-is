...
\begin{figure}[!ht]
\noindent La creazione del suddetto sequence diagram, sviluppato a partire dalla descrizione dello scenario del caso d’uso Registrazione, ha fatto sorgere la necessità di definire un metodo, specifico per la classe SistemaGestioneEventi, \textbf{verificaCredenziali(Email,Password)}, \noindent privato, per consentire a SistemaGestioneEventi di verificare che Email e Password non siano già utilizzate da un altro Utente.
\vspace{20mm}
	\centering
...
\begin{figure}[!ht]
\noindent La creazione del suddetto sequence diagram, sviluppato a partire dalla descrizione dello scenario del caso d’uso PubblicaEvento, ha fatto sorgere la necessità di definire un metodo, specifico per la classe Amministratore,\noindent\textbf{verificaValidità(Titolo,Descrizione,Data,Orario,Luogo,NumMassimoPartecipanti)}, privato, per consentire all’Amministratore di verificare che i dati inseriti inserite dall’utente amministratore siano valide.
\vspace{20mm}
	\centering
	\includegraphics[width=0.8\linewidth]{assets/casid'uso/PubblicaEvento.png}
\newline
\end{figure}
